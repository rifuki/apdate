...
class Tingkat_kelas extends CI_Controller {
	var $menu_id = "";
	var $session_data = "";
	...
	public function __Construct() {
		...
		$this->cekLogin();
		$this->own_link = admin_url('master/tingkat-kelas');
		$this->load->model('master/Tingkat_kelas_model');
		$this->table = "mt_tingkat_kelas";
		$this->judul = "Tingkat Kelas";
	}
	...
	public function index() {
		...
		$this->template->_v('master/tingkat_kelas/index', $data);
	}

	public function datatables() {
		$user_access_detail = $this->user_access_detail;
		$list = $this->Tingkat_kelas_model->get_datatables();
        $data = array();
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Tingkat_kelas_model->count_all(),
            "recordsFiltered" => $this->Tingkat_kelas_model->count_filtered(),
            "data" => $data,
        );
	...
	public function create() {
		...
		$this->template->_v('master/tingkat_kelas/form', $data);
	}

	public function edit($id) {
		$id = (int) $id;
		$this->privilege('is_update');
        $model = $this->Tingkat_kelas_model->find($id);
        if (empty($model)) {
	...
	private function validation($post_data) {
		...
		if (!empty($id)) {
			$data = $this->Tingkat_kelas_model->find($id);
			if (empty($data)) {
				$this->session->set_flashdata('error', "Data not found");
	        	return redirect($this->own_link);
	        }
