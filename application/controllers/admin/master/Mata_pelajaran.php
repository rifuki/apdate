...
class Mata_pelajaran extends CI_Controller {
	...
	public function __Construct() {
		...
		$this->cekLogin();
		$this->own_link = admin_url('master/mata-pelajaran');
		$this->load->model('master/Mata_pelajaran_model');
		$this->table = "mt_mata_pelajaran";
		$this->judul = "Mata Pelajaran";
	}
	...
	public function index() {
		...
		$this->template->_v('master/mata_pelajaran/index', $data);
	}
	...
	public function datatables() {
        ...
        foreach ($list as $field) {
            ...
            $row_menu[] = $field->code;
            $row_menu[] = $field->name;
            $row_menu[] = $field->is_ekstrakulikuler == 1 ? '<span class="badge badge-success">YA</span>' : '<span class="badge badge-secondary">TIDAK</span>';
	...
	public function create() {
		...
		$this->template->_v('master/mata_pelajaran/form', $data);
	}

	public function edit($id) {
		$id = (int) $id;
		$this->privilege('is_update');
        $model = $this->Mata_pelajaran_model->find($id);
        if (empty($model)) {
	...
	private function validation($post_data) {
		...
		if (!empty($id)) {
			$data = $this->Mata_pelajaran_model->find($id);
			if (empty($data)) {
				$this->session->set_flashdata('error', "Data not found");
	        	return redirect($this->own_link);
	        }
