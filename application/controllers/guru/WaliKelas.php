...
class WaliKelas extends CI_Controller {
	...
	public function erapor_detail() {
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			// dd($data);
			if (!empty($data)) {
				success('Data berhasil didapatkan.', ['view' => $this->load->view('guru/wali_kelas/e_rapor_detail', ['data' => $data], TRUE)]);
			}
	...
	public function erapor_detail_page() {
		$session = $this->session->userdata('user_dashboard');
		$active_periode = active_periode();
		// $data['list_siswa'] = [];
		// $data['filter'] = [
		...
		// 	}
		// }
		$data['judul'] = 'E-Rapor Detail';
		$data['subjudul'] = 'E-Rapor Detail';
		// $data['list_kelas'] = $list_kelas;
		$data['active_periode'] = $active_periode;
		// dd($data);
		$this->template->_vGuru('wali_kelas/e_rapor_detail_page', $data);
	}
