...
<div class="row">
  <div class="col-md-12">
    <div class="card">
      ...
      <div class="card-body">
        <div class="mb-3">
          <button class="btn btn-primary" data-toggle="modal" data-target="#modalAspirasi" id="btnAddAspirasi">Tambah Aspirasi</button>
        </div>
              ...
              <?php if (!empty($listdata)): ?>
                <?php $no = 1; foreach ($listdata as $row): ?>
                  <tr>
                    ...
                    <td><?= $row['deskripsi'] ?></td>
                    <td>
                      <button class="btn btn-warning btn-sm btnEditAspirasi"
                        data-id="<?= $row['id'] ?>"
                        data-judul="<?= $row['judul'] ?>"
                        data-deskripsi="<?= htmlspecialchars($row['deskripsi']) ?>"
                        data-toggle="modal"
                        data-target="#modalAspirasi">
                        Ubah
                      </button>
        ...
        <!-- Modal Tambah/Edit aspirasi -->
        <div class="modal fade" id="modalAspirasi" tabindex="-1" role="dialog" aria-labelledby="modalAspirasiLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <form id="formAspirasi">
                <div class="modal-header">
                  <h5 class="modal-title" id="modalAspirasiLabel">Tambah/Edit aspirasi</h5>
...
<script>
  ...
  // Modal form handler untuk aspirasi
  document.addEventListener('DOMContentLoaded', function() {
    // Reset modal saat tambah aspirasi
    document.getElementById('btnAddAspirasi').addEventListener('click', function() {
      document.getElementById('modalAspirasiLabel').innerText = 'Tambah Aspirasi';
      document.getElementById('id').value = '';
        tinymce.get('deskripsi').setContent('');
    });
    // Isi modal saat edit aspirasi
    document.querySelectorAll('.btnEditAspirasi').forEach(function(btn) {
      btn.addEventListener('click', function() {
        document.getElementById('modalAspirasiLabel').innerText = 'Edit Aspirasi';
    ...
    $('#formAspirasi').on('submit', function(e) {
      ...
      fetch("<?= base_url('siswa/aspirasi/save') ?>", {
          method: 'POST',
          body: formData
      })
